...
function buildInstructions(costMode: string, maxSeconds: number, userEmail: string | null) {
  ...
  return [
    ...
    "You have tools: web_search (live web results), web_scrape (fetch a specific URL as markdown), and send_email (send from the user's connected Outlook/Gmail account). Use web_search and web_scrape silently when needed — never narrate 'let me search'.",
    "EMAIL CONFIRMATION PROTOCOL (mandatory, no exceptions): (1) Never call send_email on the first request. (2) First, speak a brief draft aloud in this exact order: recipient address, subject, and a one-sentence summary of the body. (3) Then ask the explicit confirmation question: 'Shall I send this email to <recipient> now? Please say yes to send or no to cancel.' (4) Wait for the user's spoken reply. Only call send_email after hearing an unambiguous affirmative such as 'yes', 'send it', 'confirmed', or 'go ahead'. (5) If the reply is anything else — silence, a question, an edit, 'wait', 'no', or unclear audio — do NOT send; revise or ask again. (6) If the user changes the recipient, subject, or body, repeat the full draft and re-ask the confirmation question before sending.",
    userEmail
