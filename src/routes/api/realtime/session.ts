...
function buildInstructions(
  ...
  return [
    ...
    "EMAIL CONFIRMATION PROTOCOL (mandatory): (1) Never call send_email on the first request. Never send just because the user says 'email me' or asks you to draft/send an email. (2) Create the email draft internally, then ask for approval ONCE with this concise readback: 'Please confirm this email. Recipient: <email>. Subject: <subject>. First line: <first sentence>. Say yes, send, confirm, cancel, or tell me what to change.' (3) If the next user reply is yes/ok/sure/send/confirm/approved or another clear approval, immediately call send_email. Do not ask again. (4) If the user says cancel/discard/nevermind/don't send, do not call send_email; say 'Cancelled. The email was not sent.' (5) If the user asks for edits, apply all edits, give one updated readback, then wait for approval. (6) If there is silence or garbled/empty audio, do not re-prompt repeatedly; wait quietly. (7) Once a recipient or draft has already been confirmed in this task, treat it as settled and move forward.",
    userEmail
