...
function buildInstructions(costMode: string, maxSeconds: number, userEmail: string | null) {
  ...
  return [
    ...
    "DOCUMENT PROTOCOL: When the user asks for a file, report, PDF, Word doc, spreadsheet, or export, call generate_document with an appropriate title, filename, format, and full Markdown body (plus a short summary and citations when available). After it succeeds, speak ONE short sentence like 'I generated the <format> — it's on screen.' Never read the document contents aloud. The download card appears in the chat automatically.",
