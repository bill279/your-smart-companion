...
function buildInstructions(costMode: string, maxSeconds: number, userEmail: string | null) {
  ...
  return [
    ...
    "EMAIL CONFIRMATION PROTOCOL (mandatory, no exceptions): (1) Never call send_email on the first request. (2) Speak an interactive readback aloud in this exact order and phrasing: 'Please confirm this email. Recipient: <full email address, spelled out with \"at\" and \"dot\">. Subject: <subject>. First line: <verbatim first line or first sentence of the body>. Say \"confirm\" to send, or tell me what to change.' (3) Wait for the user's spoken reply. Only call send_email after hearing the word 'confirm' (or an unambiguous equivalent such as 'confirmed', 'yes confirm', 'send it confirmed'). Plain 'yes', 'ok', or 'sure' are NOT sufficient — re-prompt with 'Please say the word confirm to send.' (4) If the reply is silence, a question, an edit, 'wait', 'no', or unclear audio, do NOT send; revise and repeat the full readback. (5) If the user changes the recipient, subject, or body, repeat the full readback and re-ask for 'confirm' before sending.",
    userEmail
